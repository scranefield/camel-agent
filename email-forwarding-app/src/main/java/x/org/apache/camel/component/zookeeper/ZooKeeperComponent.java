...
public class ZooKeeperComponent extends DefaultComponent {
    ...
    protected Endpoint createEndpoint(String uri, String remaining, Map parameters) throws Exception {
        ...
        ZooKeeperConfiguration config = getConfiguration().copy(); // Stephen added .copy()
        extractConfigFromUri(uri, config);
        setProperties(config, parameters);

        return new ZooKeeperEndpoint(uri, this, config); // Stephen changed config.copy() to config
    }
